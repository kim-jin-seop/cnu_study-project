 ...
 *
 *
 * 201601989 kim jin seop <jin2609>
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void eval(char *cmdline) 
{
	char *argv[MAXARGS]; //명령저장
	pid_t pid;
	int bg; //입력
	sigset_t mask;
	
	bg = parseline(cmdline, argv);
	if(argv[0] == NULL){   //비어있는 라인무시
		return;
	}
	...
	if(!builtin_cmd(argv)){
		...
		sigaddset(&mask,SIGTSTP); //SIGTSTP 추가
		sigprocmask(SIG_BLOCK,&mask,NULL); // Block
		pid = fork();
		if(pid  == 0){ //Child Process인 경우, excve실행
			setpgid(0,0);
		...
		if(!bg){//foreground
			...
			printf("(%d) (%d) %s",pid2jid(pid), pid, cmdline); //pid 출력
		}
	}
	return;
}	
...
{
	char *cmd = argv[0];
	if(!strcmp(cmd,"quit")){ //cmd와 quit가 동일한가?
		exit(0); //쉘이 종료(같을경우)
	}
	if(!strcmp(cmd,"jobs")){ //명령이 jobs일 경우 수행
		listjobs(jobs,1);
		return 1;
	}
	if(!strcmp(cmd, "bg")){
