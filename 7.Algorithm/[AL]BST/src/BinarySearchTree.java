...
public class BinarySearchTree {
	...
	/*printInorder
	 * 중위 탐색*/
	public void printInorder(Node node, FileWriter fw) {
		if(node != null) {
			printInorder(node.getLeft(),fw);
			System.out.println(node.getData());
			try {
				fw.write(String.valueOf(node.getData() + " "));
				fw.flush();
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			printInorder(node.getRight(),fw);
		}
	}
}